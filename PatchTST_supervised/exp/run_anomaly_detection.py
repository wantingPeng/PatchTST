...
# This allows imports to work correctly when script is run from exp/ directory
...
parser.add_argument('--embed_type', type=int, default=4, help='0: default 1: value embedding + temporal embedding + positional embedding 2: value embedding + temporal embedding 3: value embedding + positional embedding 4: value embedding (recommended for anomaly detection)')
...
# 仅测试模式
    ...
    if args.checkpoint_dir:
        # 从checkpoint_dir提取目录路径（去掉末尾的model.pth）
        checkpoint_dir = os.path.dirname(args.checkpoint_dir) if os.path.isfile(args.checkpoint_dir) else args.checkpoint_dir
        Exp.latest_checkpoint_dir = checkpoint_dir
        print(f"使用指定的checkpoint: {checkpoint_dir}")
    Exp.test(args.setting)
    print("测试完成!")
